...
chrome.runtime.onMessage.addListener(
    function (request, sender, sendResponse) {
        ...
        if (request.action === 'start') {
            ...
            lute.stageMetadata(request.metadata);
        }
    });
...
lute.start = function (tab) {
    ...
    chrome.browserAction.setTitle({
        title: "Listening to " + service.name
    });
...
lute.stageMetadata = function (metadata) {
    log('Metadata collected for audio file: ' + JSON.stringify(metadata), 1);
    lute.audio.metadata = metadata;
    lute.audio.filename = getFilename(metadata);
    lute.foundLoot();
};

lute.foundLoot = function () {
    let hasUrl = !!(lute.audio.url);
    let hasMetadata = !!(lute.audio.metadata);
    if (hasUrl && hasMetadata) {
...
lute.openBox = function () {
    ...
    let openIcon = drawIcon(75, colors);
